class CashFlows::TowardsSavings
	...
	def call
		...
		if goal <= month.cash_flow.amount
			...
			savings_account.update!(amount: (savings_account.amount + goal).round(2))
			month.cash_flow.update!(amount: (month.cash_flow.amount - goal).round(2))
		else
			month.advices.create(description:"Cash flow of #{month.cash_flow.amount} should go towards your #{savings_account.name} account")
			savings_account.update!(amount: (savings_account.amount + month.cash_flow.amount).round(2))
